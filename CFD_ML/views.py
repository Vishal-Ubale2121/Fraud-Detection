...
# The following line has been added to get rid of the main thread not in loop error.
matplotlib.use('Agg')
...
def prediction(request):
    if request.method == 'POST':
        ...
        FraudDetectCount.objects.create(Filename=uploaded_file.name, Percentage=fp_percent, FalsePos=fp_count,
                                        Suspicious=f_count, NewCust=n_count)
    ...
    return render(request, 'CFD_ML/Dashboard.html', context)
...
def dashboard(request):
    return render(request, 'CFD_ML/Dashboard.html')